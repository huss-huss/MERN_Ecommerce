...
export const addProductToWishlist = (req, res) => {
  const q =
    "INSERT INTO WishlistItem (wishlist_id, product_id) VALUES (?, ?)";

  db.query(
    q,
    [req.body.wishlist_id, req.body.product_id],
    (err, data) => {
      if (err) return res.status(500).json(err);

      return res.status(200).json("Product has been added to the wishlist successfully");
    }
  );
};
...
export const removeProductFromWishlist = (req, res) => {
  ...
  db.query(q, [req.params.itemId], (err, data) => {
    ...
    return res.status(200).json("Product has been removed from the wishlist successfully");
  });
};
