...
export const addCustomProduct = (req, res) => {
  const q = "INSERT INTO CustomProduct (name, description, price) VALUES (?, ?, ?)";
...
export const updateCustomProduct = (req, res) => {
  const q = "UPDATE CustomProduct SET name = ?, description = ?, price = ? WHERE id = ?";
  ...
  db.query(
    ...
    (err, data) => {
      ...
      return res.status(200).json("Custom product has been updated successfully");
    }
  );
};
