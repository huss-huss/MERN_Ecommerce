...
export const addShoppingCartItem = (req, res) => {
  const q =
    'INSERT INTO ShoppingCartItem (user_id, product_id, quantity) VALUES (?, ?, ?)'
  ...
  db.query(
    ...
    (err, data) => {
      ...
      return res
        .status(200)
        .json('Shopping cart item has been added successfully')
    }
  )
}
...
export const updateShoppingCartItem = (req, res) => {
  const q =
    'UPDATE ShoppingCartItem SET user_id = ?, product_id = ?, quantity = ? WHERE id = ?'
  ...
  db.query(
    ...
    (err, data) => {
      ...
      return res
        .status(200)
        .json('Shopping cart item has been updated successfully')
    }
  )
}
...
export const deleteShoppingCartItem = (req, res) => {
  ...
  db.query(q, [req.params.id], (err, data) => {
    ...
    return res
      .status(200)
      .json('Shopping cart item has been deleted successfully')
  })
}
