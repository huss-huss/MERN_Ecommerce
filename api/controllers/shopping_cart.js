...
export const addProductToShoppingCart = (req, res) => {
  ...
  db.query(
    ...
    (err, data) => {
      ...
      return res.status(200).json("Product has been added to the shopping cart successfully");
    }
  );
};
...
export const removeProductFromShoppingCart = (req, res) => {
  ...
  db.query(q, [req.params.itemId], (err, data) => {
    ...
    return res.status(200).json("Product has been removed from the shopping cart successfully");
  });
};
